...
public class NoventaGrados {
	...
	public static void main(String[] args) {
		 try {
		        // Extraer el modo de deshacer del argumento pasado.
		        extraerModoDeshacer(args);

		        // Inicializar la partida con la configuración establecida.
		        inicializarPartida();

		        // Mostrar el mensaje de bienvenida e instrucciones.
		        mostrarMensajeBienvenida();
		        
		        //Mostrar tablero situación inicial
		        mostrarTablero();

		        // Bucle principal de la partida.
		        boolean partidaFinalizada = false;
		        while (!partidaFinalizada) {
		            // Recoger el texto de la jugada del jugador.
		            ...
		            if (comprobarSalir(textoJugada)) {
		                ...
		                // Comprobar si el jugador quiere deshacer la última jugada.
		                if (deshacer.consultarNumeroJugadasEnHistorico() > 0) {
		                    deshacerJugada();
		                } else {
		                    System.out.println("No hay jugadas previas que deshacer.");
		                }
		                ...
		                if (!esLegal(jugada)) {
		                    ...
		                    // Realizar la jugada empujando piezas si es necesario.
		                    realizarEmpujón(jugada);

		                    // Cambiar el turno del juego.
		                    cambiarTurnoPartida();

		                    // Mostrar el estado actualizado del tablero.
		                    mostrarTablero();

		                    // Comprobar si la partida ha terminado.
		                    partidaFinalizada = comprobarFinalizacionPartida();
		                }
		            }
		        }

		        // Mostrar el ganador al finalizar la partida.
	...
	private static void extraerModoDeshacer(String[] args) throws OpcionNoDisponibleException {
	    if (args.length > 0) {
	        configuracion = args[0].toLowerCase(); // Convertir a minúsculas para evitar problemas de mayúsculas/minúsculas.
	        if (!configuracion.equals("jugadas") && !configuracion.equals("arbitros")) {
	            throw new OpcionNoDisponibleException("El modo de deshacer proporcionado no es válido.");
	        }
	    } else {
	        throw new OpcionNoDisponibleException("No se ha proporcionado el argumento del modo de deshacer.");
	    }
	}
